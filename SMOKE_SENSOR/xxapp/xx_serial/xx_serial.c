...
#define XX_SERIAL_SMOKE_BUF_SIZE                          128
...
XxSerialReceiveDef serial_smoke;
uint8 serial_smoke_buf[XX_SERIAL_SMOKE_BUF_SIZE];
...
void XxSerialPoll(uint32_t sysTimeMillisecond)
{
	...
	#ifdef XX_SERIAL_SMOKE_PORT_NUM
		xxSerialReceive((void*)&serial_smoke, XX_SERIAL_SMOKE_PORT_NUM);
		XxSerialSmokeCommandPoll((void*)&serial_smoke, sysTimeMillisecond);
	#endif
}
...
void XxSerialInit(void)
{
    ...
    xxSerialDataConfig(&serial_smoke, XX_SERIAL_SMOKE_BUF_SIZE, serial_smoke_buf);
}
